{
  update = "25.03.2022",
  author = "Klient#1690",
  
  name = "Solus UI", 
  
  link = "https://raw.githubusercontent.com/kl1ent/CS-GO-Scripting/main/rawetrip/Solus%20UI/Solus%20UI.lua", 
  
  aviable = true
},

{
  update = "30.04.2022",
  author = "Klient#1690", 
  
  name = "Necron", 
  
  link = "https://raw.githubusercontent.com/kl1ent/CS-GO-Scripting/main/rawetrip/Necron/Necron.lua", 
  
  aviable = true
},

{
  update = "03.06.2022",
  author = "Klient#1690", 
  
  name = "Helper", 
  
  link = "https://raw.githubusercontent.com/kl1ent/CS-GO-Scripting/main/rawetrip/Helper/Helper.lua", 
  
  aviable = true,
  
  info = {
    images = {
      {name = "helper_aim", size = vector2d.new(376, 173)},
      {name = "helper_molotov", size = vector2d.new(618, 276)}
    },
    updates = {
      {"1. Reworked movement for grenades\n2. Added several types of throw (Rage, Legit)\n3. Reworked design\n4. Added support for movement locations"}
    }
  }
},

{
  class = 2,
  
  update = "03.06.2022",
  
  second_name = "Helper Data", 
  name = "helper_data", 
  
  type = "json",
  
  link = "https://raw.githubusercontent.com/kl1ent/CS-GO-Scripting/main/rawetrip/Helper/helper_data.json", 
  
  aviable = true
},
